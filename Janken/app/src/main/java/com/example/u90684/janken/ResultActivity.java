...
public class ResultActivity extends AppCompatActivity {
    ...
    private void saveData(int myHand, int comHand, int gameResult){
        ...
        int lastComHand = pref.getInt("LASE_COM_HAND",0);
        int laseGameResult = pref.getInt("GAME_RESULT",-1);

        editor.putInt("GAME_COUNT",gameCount + 1);
        if(laseGameResult == 2 && gameResult == 2) {
            //コンピュータが連勝した場合
            editor.putInt("WINNING_STREAK_COUNT",winningStreakCount + 1);
        } else {
            editor.putInt("WINNIG_STREAK_COUNT",0);
        }
